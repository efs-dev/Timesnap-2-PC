...
// Running on an Android device.
      ...
      isDaydream = (vrDeviceName == GvrSettings.VR_SDK_DAYDREAM);
        ...
        void Update()
        {
            ...
            var newPointer = activeControllerPointer;

            if (controllerPointers.Length > 1 && controllerPointers[1] != null)
            {
                // Buttons that can trigger pointer switching.
                var buttonMask = GvrControllerButton.App | GvrControllerButton.TouchPadButton;
                var trackedController1 = controllerPointers[1].GetComponent<GvrTrackedController>();
                foreach (var hand in AllHands)
                {
                    var device = GvrControllerInput.GetDevice(hand);
...
// Viewer type switched at runtime.
      ...
      SetVRInputMechanism();
        ...
        {
            var playerSettingsVrSdks = XRSettings.supportedDevices;
...
// Running on an Android device.
    private static bool IsDeviceDaydreamReady() {
      ...
      return;
            ...
            var isVrSdkListEmpty = !playerSettingsHasCardboard() && !playerSettingsHasDaydream();
            if (!isDaydream)
            {
            ...
            var vrSdkWarningMessage = isVrSdkListEmpty ? EMPTY_VR_SDK_WARNING_MESSAGE : "";
            var controllerMessage = "";
            var graphicRaycaster =
                messageCanvas.GetComponent<GvrPointerGraphicRaycaster>();
            var dominantDevice = GvrControllerInput.GetDevice(GvrControllerHand.Dominant);
            var connectionState = dominantDevice.State;
        ...
        {
            print("setVRInputMechanism: isDaydream: " + isDaydream);
            SetGazeInputActive(!isDaydream);
            SetControllerInputActive(isDaydream); // commenting here resolves problem, sort of: gaze pointer can't click
        }
        ...
        private void SetControllerInputActive(bool active)
        {
            ...
            var pointer = controllerPointers[activeControllerPointer].GetComponentInChildren<GvrLaserPointer>(true);
            if (pointer != null)
            {
                GvrPointerInputModule.Pointer = pointer;
            }
