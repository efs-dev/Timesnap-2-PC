...
namespace HutongGames.PlayMaker.Actions
{
    ...
    public class DOTweenVRPanoramaGroupFade : FsmStateAction
    {
        [RequiredField]
        [CheckForComponent(typeof(VRPanoramaGroup))]
        ...
        public override void OnEnter()
        {
            ...
            tweener = DOTween.To(() => Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<VRPanoramaGroup>().Alpha, x => Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<VRPanoramaGroup>().Alpha = x, to.Value, duration.Value);

            // Fsm.GetOwnerDefaultTarget(gameObject).GetComponent<VRPanorama>().DOFade(to.Value, duration.Value);
            ...
            if (debugThis.Value) Debug.Log("GameObject [" + State.Fsm.GameObjectName + "] FSM [" + State.Fsm.Name + "] State [" + State.Name + "] - DOTween VRPanorama Fade - SUCCESS!");
